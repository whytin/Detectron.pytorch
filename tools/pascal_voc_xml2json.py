...
def parseXmlFiles(xml_path, file_list):
    for f in os.listdir(xml_path):
        if (not f.endswith('.xml')) and (not any(f in fl for fl in file_list)):
            continue
...
if __name__ == '__main__':
    ...
    for f in os.listdir(image_sets_path):
        ...
        image_set_file = os.path.join(image_sets_path, f)

        file_list = []
        with open(image_set_file, 'r') as isf:
            lines = isf.readlines()
            for line in lines:
        ...
        print('Writing to file {0}'.format(json_file_path))
        json.dump(coco, open(json_file_path, 'w'))
